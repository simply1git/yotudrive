...
from src.settings import load_settings as load_core_settings, save_settings as save_core_settings, EngineSettings
...
class YotuDriveGUI:
    ...
    def restore_selected(self):
        ...
        def worker():
            try:
                ...
                # Pass password here
                restored = self.engine.decode_source(
                    frames_dir=download_dir,
                    output_file=output_file,
                    password=password,
                    overrides={"threads": threads},
                    progress_callback=decode_cb,
                )
    ...
    def delete_selected(self):
        ...
        if messagebox.askyesno("Confirm", f"Are you sure you want to remove '{file_name}' from the database?"):
            try:
                self.engine.db.remove_file(file_id)
    ...
    def setup_upload_tab(self):
        ...
        self.last_encoded_video_path = None
        ...
        ttk.Label(reg_box, text="Privacy:").pack(side='left', padx=(12, 4))
        self.oauth_privacy = tk.StringVar(value="unlisted")
        ttk.Combobox(reg_box, textvariable=self.oauth_privacy, values=["public", "unlisted", "private"], state="readonly", width=10).pack(side='left', padx=4)
        ttk.Button(reg_box, text="Upload Direct (OAuth)", command=self.oauth_upload_last_video, bootstyle="primary").pack(side='left', padx=6)
        ...
        def worker():
            ...
            try:
                ...
                for idx, current_file in enumerate(files_to_process):
                    ...
                    self.log(f"Starting pipeline: {base_name}")

                    video_encoder = self.setting_encoder.get()
                    result = self.engine.encode_to_video(
                        ...
                        progress_callback=lambda pct, msg: self.update_progress(pct, f"{msg}{part_str}"),
                    ...
                    self.log(f"Video created: {os.path.basename(result['video_file'])}")
    ...
    def _restore_file_internal(self, video_path, output_dir, password, auto_cleanup, threads, check_cancel_func, progress_base=0, progress_scale=1.0):
        try:
            ...
            def pipeline_cb(pct, message):
                global_pct = progress_base + (pct * progress_scale)
                self.update_progress(global_pct, f"{message} {os.path.basename(video_path)}")

            pipeline_result = self.engine.decode_video_source(
                ...
                temp_root=unique_dir,
                auto_cleanup=auto_cleanup,
            )

            final_output = pipeline_result["output_file"]
    ...
    def enable_upload_step_2(self, video_path, filename=None):
        ...
        self.last_encoded_video_path = video_path
    ...
    def oauth_upload_last_video(self):
        if not self.last_encoded_video_path or not os.path.exists(self.last_encoded_video_path):
            messagebox.showwarning("Warning", "No encoded video available. Encode and stitch a file first.")
            return

        title = self.last_encoded_filename or os.path.basename(self.last_encoded_video_path)
        privacy = self.oauth_privacy.get() or "unlisted"

        def worker():
            try:
                self.update_progress(0, "OAuth upload started...")
                uploader = YouTubeOAuthUploader()
                video_id = uploader.upload_video(
                    file_path=self.last_encoded_video_path,
                    title=title,
                    description="Uploaded via YotuDrive desktop",
                    privacy_status=privacy,
                )
                self.engine.db.add_file(title, video_id, 0, {"status": "uploaded_oauth", "video_path": self.last_encoded_video_path})
                self.refresh_file_list()
                self.update_progress(100, "OAuth upload complete")
                self.msg_queue.put(("success", ("Upload Complete", f"Video uploaded successfully.\nVideo ID: {video_id}")))
            except Exception as e:
                self.update_progress(0, "OAuth upload failed")
                self.msg_queue.put(("error", ("OAuth Upload Error", str(e))))
    ...
    def register_upload(self):
        ...
        try:
            self.engine.db.add_file(file_name, video_id, 0, {"status": "uploaded"})
    ...
    def load_settings(self):
        try:
            data = load_core_settings("settings.json")
            self.setting_block_size.set(data.block_size)
            self.setting_ecc_bytes.set(data.ecc_bytes)
            self.setting_threads.set(data.threads)
            self.setting_auto_cleanup.set(data.auto_cleanup)
            self.setting_encoder.set(data.encoder)
            self.setting_theme.set(data.theme)
            self.setting_split_size.set(data.split_size)
            ...
            self.setting_compression.set(comp_map.get(data.compression, "Fast (Deflate)"))
    ...
    def save_settings(self):
        try:
            raw_compression = self.setting_compression.get()
            compression = {
                "Store (No Compression)": "store",
                "Fast (Deflate)": "deflate",
                "Best (LZMA)": "lzma",
                "BZIP2": "bzip2",
            }.get(raw_compression, "deflate")
            ...
            save_core_settings(payload, "settings.json")
            self.engine.settings = payload
    ...
    def open_verify_tool(self):
        ...
        def task():
            try:
                report = self.engine.verify_video(video_path)
                if not report.get("valid"):
                    raise ValueError(report.get("reason", "Unknown verification error"))

                msg = f"Video: {os.path.basename(video_path)}\n"
                msg += f"Version: {report.get('version', 'Unknown')}\n"
                msg += f"Block Size: {report.get('block_size', 'Unknown')}\n"
                msg += f"ECC Bytes: {report.get('ecc_bytes', 'Unknown')}\n"
                msg += f"Header Copies: {report.get('header_copies', 'Unknown')}\n"

                self.msg_queue.put(("success", ("Valid YotuDrive Archive", msg)))
                self.log(f"Verification Success: {os.path.basename(video_path)}")
