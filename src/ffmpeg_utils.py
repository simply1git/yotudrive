import os
import sys
from .utils import NamingConvention, YotuDriveException, ErrorCodes, safe_file_operation
...
def stitch_frames(frames_dir, output_video, framerate=30, encoder="libx264", preset="medium", check_cancel=None):
    ...
    if not os.path.exists(frames_dir):
        raise YotuDriveException(f"Frames directory does not exist: {frames_dir}", ErrorCodes.FILE_NOT_FOUND)
    
    if not os.path.isdir(frames_dir):
        raise YotuDriveException(f"Path is not a directory: {frames_dir}", ErrorCodes.INVALID_PATH)
    
    # Generate proper output filename if directory is provided
    if os.path.isdir(output_video):
        base_name = os.path.basename(frames_dir)
        output_video = os.path.join(output_video, NamingConvention.generate_video_name(base_name))
    ...
    try:
        ...
        raise YotuDriveException(f"FFmpeg stitching failed: {e}", ErrorCodes.ENCODING_FAILED, e)
