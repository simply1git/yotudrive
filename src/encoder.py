...
import collections
import time
from .header_v5 import pack_header_v5, COMPRESSION_DEFLATE
...
def _select_executor_class():
    """Choose a safe executor strategy for the current runtime."""
...
class Encoder:
    ...
    def run(self):
        ...
        # Process pools in frozen Windows executables can terminate abruptly due to
        # child process bootstrap/import constraints. Use a thread pool there for
        # reliability; keep process pool for normal Python execution.
        executor_cls = _select_executor_class()
        if executor_cls is concurrent.futures.ThreadPoolExecutor:
            print("Frozen Windows build detected: using ThreadPoolExecutor for stability.")
    ...
    def create_header(self, payload_length, original_size, checksum, flags, salt, filename, version=4):
        ...
        if version == 5:
            return pack_header_v5(
                ...
                salt=salt,
                filename=filename,
                header_copies=DEFAULT_HEADER_COPIES,
