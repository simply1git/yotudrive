...
def encode(args):
    ...
    try:
        result = engine.encode_file(input_file=input_file, output_root=output, password=password)
        print(f"Success! Encoded {len(result.parts)} part(s).")
        for part in result.parts:
            print(f"- Part frames: {part.frames_dir}")
...
def decode(args):
    ...
    try:
        restored = engine.decode_source(frames_dir=input_dir, output_file=output, password=password)
        print(f"Success! File restored to {restored}")
    except Exception as e:
