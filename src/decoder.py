...
import base64
import re
from .header_v5 import parse_header_v5
...
class Decoder:
    ...
    def parse_header(self, data):
        ...
        if version == 4:
            ...
            parsed = parse_header_v5(data[:HEADER_SIZE])
            hdr = parsed.header
            if not parsed.crc_valid:
                print("Warning: Header CRC mismatch for V5 header")
                valid_crc = False
    ...
    def detect_config(self, frame_files):
        ...
        candidates = self._candidate_block_sizes(max_block_size=64)
        
        for frame_path in frames_to_check:
            # Try common block sizes
            for test_block_size in candidates:
                try:
    ...
    @staticmethod
    def _candidate_block_sizes(max_block_size=64):
        """Return likely block sizes that evenly divide frame dimensions."""
        valid = [
            s
            for s in range(1, max_block_size + 1)
            if VIDEO_WIDTH % s == 0 and VIDEO_HEIGHT % s == 0
        ]
        priority = [1, 2, 3, 4, 5, 6, 8, 10, 12, 15, 16, 20, 24, 30, 32, 40, 48, 60, 64]
        ordered = [s for s in priority if s in valid]
        ordered.extend([s for s in valid if s not in ordered])
        return ordered
